//Function overload holo ekoi function-er bibhinno bhabe use kora.
...
type SMSNotification = { to: string; message: string };

//example 2
// Overload signatures
function sendNotification(notification: EmailNotification): void;
function sendNotification(notification: SMSNotification): void;
function sendNotification(notifications: (EmailNotification | SMSNotification)[]): void;

// Implementation
function sendNotification(input: EmailNotification | SMSNotification | (EmailNotification | SMSNotification)[]): void {
  if (Array.isArray(input)) {
    console.log("Sending multiple notifications:");
    input.forEach(n => {
      if ('subject' in n) console.log(`Email to ${n.to}: ${n.subject}`);
      else console.log(`SMS to ${n.to}: ${n.message}`);
    });
  } else {
    if ('subject' in input) console.log(`Email to ${input.to}: ${input.subject}`);
    else console.log(`SMS to ${input.to}: ${input.message}`);
  }
}
...
//exaple 3
// Overloads
function area(radius: number): number;                 // circle
function area(length: number, width: number): number;  // rectangle
function area(sides: number[]): number;               // polygon (regular)

// Implementation
function area(a: number | number[], b?: number): number {
  if (Array.isArray(a)) {
...
function log(value: boolean, prefix: string): string;
function log(values: (string | number)[]): string[];

// Implementation
function log(value: string | number | boolean | (string | number)[], prefix?: string): string | number | string[] {
